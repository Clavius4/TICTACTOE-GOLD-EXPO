...
const db = SQLite.openDatabase('tictactoe_stats.db');
...
function initializeDatabase() {
    db.transaction((tx) => {
        ...
        // Check if any data exists in the game_stats table
        tx.executeSql(
          'SELECT * FROM game_stats LIMIT 1', // Try to select one row
          [],
          (_, { rows }) => {
            if (rows.length === 0) {
              // If no data exists, insert the initial data
              tx.executeSql(
                'INSERT INTO game_stats (games_played, games_won, games_tied, win_percentage, max_win_in_a_row, min_victory_time, time_played) VALUES (?, ?, ?, ?, ?, ?, ?)',
                [0, 0, 0, 0, 0, 'N/A', 'N/A']
              );
            }
          },
